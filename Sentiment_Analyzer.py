...
log_path = os.path.join(os.path.expanduser("~"), "SentimentAnalyzer.log")
...
# *** Security Warning ***
# Replace the following `GOOGLE_CREDENTIALS_JSON` with a secure method of storing credentials.
# For example, load from environment variables or an external JSON file excluded from version control.
# It is highly recommended to use an external JSON file for credentials.
...
CURRENT_VERSION = "1.0.0"  # Update this as you release new versions
VERSION_URL = "https://yourserver.com/Sentiment_Analyzer_version.txt"  # URL to the version.txt file
...
def authorize_gsheets():
    """
    Authorizes and returns the first sheet of the Google Spreadsheet.
    """
    try:
...
def save_mood_to_gsheets(user_id, selected_mood, date, time_str, work_related=None, reason=None):
    """
    Saves the user's mood data to Google Sheets.
    """
    try:
...
def check_for_updates():
    """
    Checks if a new version is available by comparing the current version with the latest version from VERSION_URL.
    Returns a tuple (is_update_available, latest_version, download_url).
    """
    try:
        ...
        latest_version = data.get(VERSION_KEY)
        download_url = data.get(DOWNLOAD_URL_KEY)
...
def download_update(download_url):
    ...
    Saves it as 'Sentiment_Analyzer_latest.py'.
    Returns True if successful, False otherwise.
    """
    try:
        response = requests.get(download_url)
        response.raise_for_status()
        with open("Sentiment_Analyzer_latest.py", "wb") as f:
            f.write(response.content)
        logging.info("Downloaded the latest version successfully.")
        return True
    except Exception as e:
...
def apply_update():
    """
    Replaces the current script with the downloaded latest version and restarts the application.
    """
    try:
        # Replace the current script with the new one
        os.replace("Sentiment_Analyzer_latest.py", sys.argv[0])
        logging.info("Updated the application successfully.")
        # Restart the application
        os.execv(sys.executable, [sys.executable] + sys.argv)
    except Exception as e:
        logging.error(f"Error applying update: {e}")

def auto_update_process(root):
    """
    Runs the auto-update process. If an update is available, prompts the user to update.
    """
    updated, latest_version, download_url = check_for_updates()
    if updated:
        def prompt_update():
...
class MoodTrackerApp:
    def __init__(self, master):
        ...
        # Schedule the first popup after a short delay (e.g., 7 hours)
        self.master.after(25200000, self.show_popup)  # 25200000 ms = 7 Hours

    def disable_close(self):
        """
        Prevents the window from being closed without submitting a response.
        """
        messagebox.showwarning("Warning", "You cannot close this window without submitting a response.")

    def make_modal(self):
        """
        Makes the window modal by disabling other windows and focusing on this one.
        """
    ...
    def create_widgets(self):
        """
        Creates all the widgets in the application, including the header, mood buttons, and submit button.
        """
    ...
    def show_popup(self):
        """
        Resets the UI and brings the window to focus for user input.
        """
    ...
    def select_mood(self, mood):
        """
        Handles mood selection by the user.
        """
    ...
    def on_work_related_selected(self, event):
        """
        Handles the selection in the work-related dropdown.
        """
    ...
    def submit_response(self):
        """
        Submits the user's response to Google Sheets after validation.
        """
        ...
        # Schedule the next popup after 7 Hours (25200000 ms)
        self.master.after(25200000, self.show_popup)  # 25200000 ms = 7 Hours
...
def run_application():
    ...
    # Check for updates in a separate thread to avoid blocking the UI
    update_thread = threading.Thread(target=auto_update_process, args=(root,), daemon=True)
    update_thread.start()
